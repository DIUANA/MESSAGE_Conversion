...
*    EMISSV(node,emission,type_tec,year_all)       aggregate emissions by technology type and land-use model emulator
;
...
* auxiliary variable for left-hand side of relations (linear constraints)
    ...
    REL2(relation2,node,year_all)                  auxiliary variable for left-hand side of user-defined relations
...
Positive variables
    ...
    SLACK_RELATION_BOUND_UP2(relation2,node,year_all)   slack variable for upper bound of generic relation
    SLACK_RELATION_BOUND_LO2(relation2,node,year_all)   slack variable for lower bound of generic relation
    SLACK_CAP_CON(node,tec,vintage,year_all,time)
    SLACK_MUF_TIME(node,tec,vintage,year_all,time)
    SLACK_MUF(node,tec,vintage,year_all)
    SLACK_CAP_LAND(node,tec,year_all,BLUES_type_land_tec)    
    SLACK_EMISSION(node,type_emission,type_tec,type_year)
    SLACK_EXT_BOUND_UP(node,commodity,grade,year_all)    
;
...
*    EMISSION_EQUIVALENCE2            auxiliary equation to simplify the notation of emissions
    ...
    BLUES_LAND_BALANCE              testing constraint to guarantee the balance between the land use technologies
***    LDR_ADJ                         adding sub annual min utilization factor to make LDR works
**    BLUES_LAND_HIST_BAL
    RELATION_EQUIVALENCE2
    RELATION_CONSTRAINT_UP2
    RELATION_CONSTRAINT_LO2
    MIN_UTILIZATION_CONSTRAINT_TIME
;
*----------------------------------------------------------------------------------------------------------------------*
* equation statements                                                                                                  *
*----------------------------------------------------------------------------------------------------------------------*
...
* technology capacity investment, maintainance, operational cost
    + SUM((tec)$( map_tec(node,tec,year) ),
            ...
            + SUM((vintage,mode,time)$( map_tec_lifetime(node,tec,vintage,year) AND map_tec_act(node,tec,year,mode,time) ),
                ...
                round(
                    var_cost(node,tec,vintage,year,mode,time) 
                *  duration_time(time)
*vom no BLUES is the value to produce the main output amount ?????????
                *  main_output_val(node,tec,vintage,year,mode,time) 
                ,6)
*            + SUM((vintage,mode)$( map_tec_lifetime(node,tec,vintage,year) ),
*                var_cost2(node,tec,vintage,year,mode) * 
*                SUM(time$(map_tec_act(node,tec,year,mode,time)),     
*                        ACT(node,tec,vintage,year,mode,time) 
*                *
*                round(                           
*                        (main_output_val(node,tec,vintage,year,mode,time)   
...
* cost terms associated with linear relations2
    + SUM(relation2$( relation_cost2(relation2,node,year) ),
        relation_cost2(relation2,node,year) * REL2(relation2,node,year) )
...
%SLACK_CAP_TOTAL_BOUND_LO%  + 10 * SLACK_CAP_TOTAL_BOUND_LO(node,tec,year)
        ) * ABS( 1e5 + inv_cost(node,tec,year) ) )
...
%SLACK_RELATION_BOUND_LO% + 1e6 * SLACK_RELATION_BOUND_LO(relation,node,year)$( is_relation_lower(relation,node,year) )
    ...
    + SUM((relation2), 0
%SLACK_RELATION_BOUND_UP2% + 1e6 * SLACK_RELATION_BOUND_UP2(relation2,node,year)$( is_relation_upper2(relation2,node,year) )
%SLACK_RELATION_BOUND_LO2% + 1e6 * SLACK_RELATION_BOUND_LO2(relation2,node,year)$( is_relation_lower2(relation2,node,year) )
        )
    + SUM((tec,vintage, mode, time)$( map_tec_time(node,tec,year,time) AND  
    map_tec_lifetime(node,tec,vintage,year) AND 
    map_tec_act(node,tec,year,mode,time)), ( 0
%SLACK_CAP_CON% + 1e6 * SLACK_CAP_CON(node,tec,vintage,year,time)
%SLACK_MUF_TIME% + 1e6 * SLACK_MUF_TIME(node,tec,vintage,year,time)
                )
            )
    + SUM((tec, vintage)$( map_tec(node,tec,year) AND  map_tec_lifetime(node,tec,vintage,year) ), ( 0
%SLACK_MUF% + 1e6 * SLACK_MUF(node,tec,vintage,year)
                )
            )
    + SUM((tec,BLUES_type_land_tec) $ ( map_BLUES_land_rel(node,tec, BLUES_type_land_tec) ) , (0
%SLACK_CAP_LAND% + 1e6 * SLACK_CAP_LAND(node,tec,year,BLUES_type_land_tec)   
                )
            )
    + SUM((type_emission,type_tec,type_year)$is_bound_emission(node,type_emission,type_tec,type_year), (0
%SLACK_EMISSION% + 1e6   * SLACK_EMISSION(node,type_emission,type_tec,type_year)
                )
            )
    + SUM((commodity,grade)$( map_resource(node,commodity,grade,year) ), (0
%SLACK_EXT_BOUND_UP% + 1e6   * SLACK_EXT_BOUND_UP(node,commodity,grade,year)
                )
            )
;
...
*        *  duration_time(time_act) 
        * duration_time_rel(time_od,time_act)
        ) ;
...
* remaining resources multiplied by remaining-resources-factor
    ...
    * ( resource_volume(node,commodity,grade)
        - SUM(year2$( year_order(year2) < year_order(year) ),
            duration_period2(year2) * EXT(node,commodity,grade,year2) ) ) ;
...
RESOURCE_HORIZON(node,commodity,grade)$( SUM(year$map_resource(node,commodity,grade,year), 1 ) )..
    SUM(year, duration_period2(year) * EXT(node,commodity,grade,year) 
%SLACK_EXT_BOUND_UP% - SLACK_EXT_BOUND_UP(node,commodity,grade,year)  
    )  
        =L= resource_volume(node,commodity,grade) ;
...
STOCKS_BALANCE(node,commodity,level,year)$( map_stocks(node,commodity,level,year) )..
    ...
    duration_period2(year) * SUM(time$( map_commodity(node,commodity,level,year,time) ),
...
CAPACITY_CONSTRAINT(node,inv_tec,vintage,year,time)$( map_tec_time(node,inv_tec,year,time)
        AND map_tec_lifetime(node,inv_tec,vintage,year) )..
    SUM(mode$( map_tec_act(node,inv_tec,year,mode,time) ), ACT(node,inv_tec,vintage,year,mode,time) *
                                                main_output_val(node,inv_tec,vintage,year,mode,time)
                                                )
%SLACK_CAP_CON% - SLACK_CAP_CON(node,inv_tec,vintage,year,time)
        =L=
*        duration_time(time) *
        capacity_factor(node,inv_tec,vintage,year,time) * CAP(node,inv_tec,vintage,year) ;
***
...
CAPACITY_MAINTENANCE_HIST(node,inv_tec,vintage,first_period)$( map_tec_lifetime(node,inv_tec,vintage,first_period)
    ...
    =L= remaining_capacity(node,inv_tec,vintage,first_period) *
        duration_period2(vintage) * historical_new_capacity(node,inv_tec,vintage) ;
...
CAPACITY_MAINTENANCE_NEW(node,inv_tec,vintage,vintage)$( map_tec_lifetime(node,inv_tec,vintage,vintage) )..
    ...
    =E= remaining_capacity(node,inv_tec,vintage,vintage)
        * duration_period2(vintage) * CAP_NEW(node,inv_tec,vintage) ;
...
***
* Equation CAPACITY_MAINTENANCE NO EARLY RETIREMENT
* """""""""""""""""""""""""""""
* The third constraint implements the dynamics of capacity maintenance throughout the model horizon.
* Installed capacity must be maintained over time until decommissioning, which is irreversible.
*
*   .. math::
*      CAP_{n,t,y^V,y} & \leq
*          remaining\_capacity_{n,t,y^V,y} \cdot
...
*
***
*CAPACITY_MAINTENANCE_NO_ER(node,no_er_tec,vintage,year)$( map_tec_lifetime(node,no_er_tec,vintage,year)
*        AND NOT first_period(year) AND year_order(vintage) < year_order(year))..
*    CAP(node,no_er_tec,vintage,year)
*    =G= remaining_capacity(node,no_er_tec,vintage,year) *
*        ( SUM(year2$( seq_period(year2,year) ),
*              CAP(node,no_er_tec,vintage,year2) ) ) ;
***
...
MIN_UTILIZATION_CONSTRAINT(node,inv_tec,vintage,year)$( map_tec_lifetime(node,inv_tec,vintage,year)
    ...
    SUM((mode,time)$( map_tec_act(node,inv_tec,year,mode,time) ),
                                            duration_time(time) *
                                            ACT(node,inv_tec,vintage,year,mode,time) *
                                            main_output_val(node,inv_tec,vintage,year,mode,time))
%SLACK_MUF% + SLACK_MUF(node,inv_tec,vintage,year)
                            =G=
        min_utilization_factor(node,inv_tec,vintage,year) * CAP(node,inv_tec,vintage,year) ;

*----------------------------------------------------------------------------------------------------------------------*
***
* Equation MIN_UTILIZATION_CONSTRAINT BY TIME
* """""""""""""""""""""""""""""""""""
* This constraint provides a lower bound on the total utilization of installed capacity over a year.
*
*   .. math::
...
*
***
MIN_UTILIZATION_CONSTRAINT_TIME(node,inv_tec,vintage,year,time)$( map_tec_lifetime(node,inv_tec,vintage,year)
        AND min_utilization_time_factor(node,inv_tec,vintage,year,time) )..
    SUM(mode$( map_tec_act(node,inv_tec,year,mode,time) ), ACT(node,inv_tec,vintage,year,mode,time) *
                                                main_output_val(node,inv_tec,vintage,year,mode,time)
                                                )
%SLACK_MUF_TIME% + SLACK_MUF_TIME(node,inv_tec,vintage,year,time)
        =G=
*        duration_time(time) *
        min_utilization_time_factor(node,inv_tec,vintage,year,time) * CAP(node,inv_tec,vintage,year) ;
...
*
*ORIGINAL EQUATION DOES NOT INCLUDE: "* main_output_val(node,tec,vintage,year,mode,time)"
*
***
...
* emissions from technology activity
        ...
        emission_factor(location,tec,vintage,year,mode,emission) * ACT(location,tec,vintage,year,mode,time) 
                  *  main_output_val(location,tec,vintage,year,mode,time)          
        * duration_time(time)
        )
...
EMISSION_CONSTRAINT(node,type_emission,type_tec,type_year)$is_bound_emission(node,type_emission,type_tec,type_year)..
    SUM( (year_all2,emission)$( cat_year(type_year,year_all2) AND cat_emission(type_emission,emission) ),
        emission_duration_period(year_all2) * emission_scaling(type_emission,emission) *
...
%SLACK_EMISSION% - SLACK_EMISSION(node,type_emission,type_tec,type_year)
...
***
* Equation BLUES_LAND_BALANCE
* """""""""""""""""""""""""""""""""""
* This constraint provides land capacity balance over years.
*
*   .. math::
*
*
*
* This constraint is only active if : is defined.
***
BLUES_LAND_BALANCE(node,tec,year,BLUES_type_land_tec) $ ( map_BLUES_land_rel(node,tec, BLUES_type_land_tec)
         AND NOT historical(year)
         AND NOT first_period(year)
                 ) ..
* capacity of land primary tec in y+1
        (SUM(vintage $ (year_order(vintage) <= year_order(year)
                         AND map_tec_lifetime(node,tec,vintage,year)), CAP(node,tec,vintage,year)   ) )
        =E=
        ( SUM((vintage, year_all2, BLUES_sec_land_tec) $( seq_period(year_all2,year)
         AND BLUES_cat_land_tec(BLUES_type_land_tec, BLUES_sec_land_tec) AND year_order(vintage) < year_order(year)),
                remaining_capacity(node,BLUES_sec_land_tec,vintage,year_all2) * CAP(node,BLUES_sec_land_tec,vintage,year_all2) ))
%SLACK_CAP_LAND% + SLACK_CAP_LAND(node,tec,year,BLUES_type_land_tec)                  
                ;
***
* Equation LAND CAPACITY MAINTENANCE HIST
* """""""""""""""""""""""""""""""""""
* This constraint provides land capacity balance in the first year
*
*   .. math::
*
*
...
* This constraint is only active if : is defined.
***
*BLUES_LAND_HIST_BAL(node,tec,vintage,first_period)$( map_tec_lifetime(node,tec,vintage,first_period)
*        AND historical(vintage) AND BLUES_land_tec(tec) AND seq_period(vintage,first_period))..
*    CAP(node,tec,vintage,first_period) + CAP(node,tec,first_period,first_period)
*    =E=
*         remaining_capacity(node,tec,vintage,first_period) *
*        duration_period(vintage) * historical_new_capacity(node,tec,vintage) ;
...
*            *  main_rel_val(relation2, location,tec,year_all2,mode,time)
            * ( SUM(vintage$( map_tec_lifetime(location,tec,vintage,year_all2) ),
...
*      REL_{r,n,y} \leq relation\_upper_{r,n,y}
***
RELATION_CONSTRAINT_UP2(relation2,node,year)$( is_relation_upper2(relation2,node,year) )..
    REL2(relation2,node,year)
%SLACK_RELATION_BOUND_UP2% - SLACK_RELATION_BOUND_UP2(relation2,node,year)
    =L= relation_upper2(relation2,node,year) ;
...
*      REL_{r,n,y} \geq relation\_lower_{r,n,y}
***
RELATION_CONSTRAINT_LO2(relation2,node,year)$( is_relation_lower2(relation2,node,year) )..
    REL2(relation2,node,year)
%SLACK_RELATION_BOUND_LO2% + SLACK_RELATION_BOUND_LO2(relation2,node,year)
    =G= relation_lower2(relation2,node,year) ;
*----------------------------------------------------------------------------------------------------------------------*
* model statements                                                                                                     *
*----------------------------------------------------------------------------------------------------------------------*

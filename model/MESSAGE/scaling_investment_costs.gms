...
*    + sum(mode$( map_tec_act(node,tec,year,mode,time) ), var_cost(node,tec,year,year,mode,time))        
    + sum(mode$( map_tec_act(node,tec,year,mode,time) ), var_cost(node,tec,year,year,mode,time) *  duration_time(time) *  main_output_val(node,tec,year,year,mode,time)  )
*    + sum(mode$( map_tec_act(node,tec,year,mode,time) ), var_cost2(node,tec,year,year,mode) )
;
...
* compute the discount factor of the very last year (not period) in the model horizon
        df_year(last_period) * POWER( 1 / ( 1 + interestrate(last_period) ), duration_period2(last_period) ) * (
...
remaining_capacity(node,tec,vintage,year_all)$( map_tec_lifetime(node,tec,vintage,year_all)
        AND ( technical_lifetime(node,tec,vintage) - duration_period_sum(vintage,year_all) < duration_period2(year_all) )
        AND ( technical_lifetime(node,tec,vintage) - duration_period_sum(vintage,year_all) > 0 ) )
    = ( technical_lifetime(node,tec,vintage) - duration_period_sum(vintage,year_all) ) / duration_period2(year_all) ;

# set default to 1 (assume that the full capacity is available over the entire period)
remaining_capacity(node,tec,historical,first_period)$( map_tec_lifetime(node,tec,historical,first_period) ) = 1 ;

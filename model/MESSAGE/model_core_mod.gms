...
* technology capacity investment, maintainance, operational cost
    + SUM((tec)$( map_tec(node,tec,year) ),
            ( round(inv_cost(node,tec,year) * construction_time_factor(node,tec,year)
                * end_of_horizon_factor(node,tec,year),6) * CAP_NEW(node,tec,year)
...
%SLACK_LAND_TYPE_LO% + 1e6 * SLACK_LAND_TYPE_LO(node,year,land_type)
    ...
    + SUM((relation), 0
%SLACK_RELATION_BOUND_UP% + 999999 * SLACK_RELATION_BOUND_UP(relation,node,year)$( is_relation_upper(relation,node,year) )
%SLACK_RELATION_BOUND_LO% + 999999 * SLACK_RELATION_BOUND_LO(relation,node,year)$( is_relation_lower(relation,node,year) )
        )
;
...
CAPACITY_CONSTRAINT(node,inv_tec,vintage,year,time)$( map_tec_time(node,inv_tec,year,time)
    ...
    SUM(mode$( map_tec_act(node,inv_tec,year,mode,time)  ), 
                                ACT(node,inv_tec,vintage,year,mode,time) * 
                                                main_output_val(node,inv_tec,vintage,year,mode,time) )
        =L=  capacity_factor(node,inv_tec,vintage,year,time) * CAP(node,inv_tec,vintage,year) ;
...
OPERATION_CONSTRAINT(node,inv_tec,vintage,year)$( map_tec_lifetime(node,inv_tec,vintage,year)
    ...
    SUM((mode,time)$( map_tec_act(node,inv_tec,year,mode,time) ), 
                                                        ACT(node,inv_tec,vintage,year,mode,time) *
                                                                  main_output_val(node,inv_tec,vintage,year,mode,time)   ) =L=
...
MIN_UTILIZATION_CONSTRAINT(node,inv_tec,vintage,year)$( map_tec_lifetime(node,inv_tec,vintage,year)
    ...
    SUM((mode,time)$( map_tec_act(node,inv_tec,year,mode,time)  ), 
                                                            duration_time(time) * ACT(node,inv_tec,vintage,year,mode,time) *
                                                                        main_output_val(node,inv_tec,vintage,year,mode,time)) =G=
...
BLUES_LAND_HIST_BAL(node,tec,vintage,first_period)$( map_tec_lifetime(node,tec,vintage,first_period)
    ...
    CAP(node,tec,vintage,first_period)
    =E=

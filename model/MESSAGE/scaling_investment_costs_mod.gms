...
*display beyond_horizon_lifetime;
***
...
*mannualy modified (line just after that and line with map_tec_vtg) 
*map_tec_vtg set the equation to be active only for tecs with defined yeat in vtgs
    (
...
construction_time_factor(node,inv_tec,year)$( map_tec(node,inv_tec,year) AND construction_time(node,inv_tec,year) ) =
     round(( 1 + interestrate(year) ) ** construction_time(node,inv_tec,year),6) ;
...
end_of_horizon_factor(node,inv_tec,vintage)$( map_tec_vtg(node,inv_tec,vintage) ) =
...
*end_of_horizon_factor(node,inv_tec,vintage)$( map_tec(node,inv_tec,vintage) ) =
    round(sum(year_all$( map_tec_lifetime(node,inv_tec,vintage,year_all) ), df_period(year_all)  )
    / ( sum(year_all$( map_tec_lifetime(node,inv_tec,vintage,year_all) ), df_period(year_all) )
        + beyond_horizon_factor(node,inv_tec,vintage) ), 6) ;

...
*                var_cost(node,tec,vintage,year,mode) * SUM(time, ACT(node,tec,vintage,year,mode,time) )
                )
            )
...
%SLACK_CAP_TOTAL_BOUND_LO%  + 10 * SLACK_CAP_TOTAL_BOUND_LO(node,tec,year)
        ) * ABS( 1e7 + inv_cost(node,tec,year) ) )
...
%SLACK_RELATION_BOUND_LO% + 1e6 * SLACK_RELATION_BOUND_LO(relation,node,year)$( is_relation_lower(relation,node,year) )
    ...
    + SUM((relation2), 0
%SLACK_RELATION_BOUND_UP2% + 1e8 * SLACK_RELATION_BOUND_UP2(relation2,node,year)$( is_relation_upper2(relation2,node,year) )
%SLACK_RELATION_BOUND_LO2% + 1e8 * SLACK_RELATION_BOUND_LO2(relation2,node,year)$( is_relation_lower2(relation2,node,year) )
        )
...
%SLACK_CAP_CON% + 1e8 * SLACK_CAP_CON(node,tec,vintage,year,time)
%SLACK_MUF_TIME% + 1e8 * SLACK_MUF_TIME(node,tec,vintage,year,time)
                )
            )
    + SUM((tec, vintage)$( map_tec(node,tec,year) AND  map_tec_lifetime(node,tec,vintage,year) ), ( 0
%SLACK_MUF% + 1e8 * SLACK_MUF(node,tec,vintage,year)
                )
            )
    + SUM((tec,BLUES_type_land_tec) $ ( map_BLUES_land_rel(node,tec, BLUES_type_land_tec) ) , (0
%SLACK_CAP_LAND% + 1e8 * SLACK_CAP_LAND(node,tec,year,BLUES_type_land_tec)   
                )
            )
    + SUM((type_emission,type_tec,type_year)$is_bound_emission(node,type_emission,type_tec,type_year), (0
%SLACK_EMISSION% + 1e8   * SLACK_EMISSION(node,type_emission,type_tec,type_year)
                )
            )
;

...
*$LOAD inv_tec, renewable_tec, no_er_tec
$LOAD balance_equality
$LOAD shares
$LOAD addon, type_addon, cat_addon, map_tec_addon
$LOAD storage_tec, level_storage, map_tec_storage
* loading new parameters and sets
$LOAD BLUES_land_tec, BLUES_sec_land_tec, BLUES_type_land_tec, BLUES_cat_land_tec, map_BLUES_land_rel
$LOAD relation2
$LOAD map_relation2, map_tec_vtg_act, map_tec_vtg
$GDXIN

Execute_load '%in%'
* general parameters
duration_period, duration_period2, duration_time, interestrate, emission_duration_period
...
main_input_val,
*main_rel_val,
min_utilization_time_factor,
capacity_factor, operation_factor, min_utilization_factor,  inv_cost, fix_cost, var_cost, var_cost2,
...
* parameters for generic relations 2 (linear constraints)
is_relation_upper2, is_relation_lower2, relation_upper2, relation_lower2,
relation_cost2, relation_total_capacity2, relation_new_capacity2, relation_activity2,
* energy stocks
commodity_stock,
* demand parameters
demand_fixed=demand
...
*map_tec_lifetime(node,tec,vintage,year_all)$( map_tec_vtg_act(node,tec,vintage,year_all) ) = yes ;
*the equation above was manually modified the one below is the original one
...
*map_tec_lifetime(node,tec,vintage,year_all)$( map_tec_vtg_act(node,tec,vintage,year_all)
*the equation above was manually modified the one below is the original one
...
display map_tec;
display map_period;
display historical_new_capacity;
display duration_period_sum;
display first_period;
display technical_lifetime;
*Parameter aa(node,tec,year_all);
*aa(node,tec,year_all)=sum(first_period, technical_lifetime(node,tec,first_period) );
...
loop((node,inv_tec,year_all)$( map_tec_vtg(node,inv_tec,year_all ) ),
    if ( technical_lifetime(node,inv_tec,year_all) <= 0 ,
        put_utility 'log'/" Error: Technical lifetime not defined for '"node.tl:0"|"inv_tec.tl:0"|"year_all.tl:0"' !" ;
        check = 1 ;
...
*round was manually added
loop(lvl_temporal,
    loop(time2$( sum(time, map_temporal_hierarchy(lvl_temporal,time,time2) ) ),
        check = 1$( round( sum( time$( map_temporal_hierarchy(lvl_temporal,time,time2) ),
            duration_time(time) ), 4) ne duration_time(time2) ) ;
    ) ;
) ;
if (check,

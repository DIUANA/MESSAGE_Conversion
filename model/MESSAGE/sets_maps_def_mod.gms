...
Sets
    ...
    no_er_tec(tec)                          technologies with no early retirement
...
* New sets fro BLUES
    BLUES_land_tec(tec)
    BLUES_sec_land_tec(tec)                       technologies used for land use as secondary level
    BLUES_type_land_tec                           types of land technologies
    BLUES_cat_land_tec(BLUES_type_land_tec, BLUES_sec_land_tec)        mapping of land technologies to respective land categories
    map_BLUES_land_rel(node, tec,BLUES_type_land_tec)                mapping of types of land technologies to the underlying parent technology   
    relation2        generic linear relations     
    map_relation2(relation2,node,year_all)             mapping of generic (user-defined) relations to nodes and years 
    is_relation_upper2(relation2,node,year_all)     flag whether upper bounds exists for generic relation
    is_relation_lower2(relation2,node,year_all)     flag whether lower bounds exists for generic relation
;
...
* map_time_period and map_tec_storage are defined at the bottom of this file.
    ...
    map_tec_vtg(node,tec,vintage)                   mapping of technology to node and years
    map_tec_vtg_act(node,tec,vintage, year_all)                   mapping of technology to node and years

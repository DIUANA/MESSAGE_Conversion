...
df_period(year_all) =
    df_year(year_all) * (
* multiply the per-year discount factor by the geometric series of over the duration of the period
        ( ( POWER( 1 + interestrate(year_all) , duration_period2(year_all) ) - 1 )
	/ interestrate(year_all) )$( interestrate(year_all) )
* if interest rate = 0, multiply by the number of years in that period
        + ( duration_period2(year_all) )$( interestrate(year_all) eq 0 ) )
;
*display duration_period;
*display df_period;
...
*  the original ORD(year_all) <= ORD(year_all3) AND ORD(year_all3) < ORD(year_all2)
duration_period_sum(year_all,year_all2) =
    SUM(year_all3$( ORD(year_all) <= ORD(year_all3) AND ORD(year_all3) < ORD(year_all2) ) , duration_period2(year_all3) ) ;
* the above one is the original the below one is the mod
*    SUM(year_all3$( ORD(year_all) <= ORD(year_all3) AND ORD(year_all3) < ORD(year_all2) ) , duration_period2(year_all3) ) ;
...
*display elapsed_years;
*display last_period;

* auxiliary parameter for duration until the end of the model horizon - including the last period
*the original value is <=
remaining_years(year_all) = SUM(year_all2$( ORD(year_all) <= ORD(year_all2) ) , duration_period2(year_all2) ) ;
*remaining_years(last_period) = duration_period2(last_period);
*remaining_years(last_period) = 0;
*display remaining_years;
